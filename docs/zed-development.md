...
- `FastEditor: Swift Build` runs only SwiftPM build with the selected Xcode
  developer directory and SDK.
...
- Xcode for SwiftPM. The tasks intentionally resolve SwiftPM through the
  selected Xcode developer directory so they do not accidentally import a newer
  Command Line Tools SDK that the active Swift compiler cannot read.
